...
parser.add_argument(
    "--s_init", type=str, required=True, help="initial parameters of student model"
)
parser.add_argument(
    "--t_model", type=str, required=True, help="path name of teacher model"
)
...
parser.add_argument(
    "--t_name", type=str, required=True, help="name of teacher"
)  # resnet20/resnet110
parser.add_argument(
    "--s_name", type=str, required=True, help="name of student"
)  # resnet20/resnet110
...
parser.add_argument(
    "--lambda_kd", type=float, default=1.0, help="trade-off parameter for kd loss"
)
...
parser.add_argument(
    "--w_dist", type=float, default=25.0, help="weight for RKD distance"
)
...
parser.add_argument(
    "--gamma", type=float, default=0.4, help="gamma in Gaussian RBF for CC"
)
parser.add_argument(
    "--P_order",
    type=int,
    default=2,
    help="P-order Taylor series of Gaussian RBF for CC",
)
parser.add_argument(
    "--w_irg_vert", type=float, default=0.1, help="weight for IRG vertex"
)
parser.add_argument("--w_irg_edge", type=float, default=5.0, help="weight for IRG edge")
parser.add_argument(
    "--w_irg_tran", type=float, default=5.0, help="weight for IRG transformation"
)
parser.add_argument(
    "--sf",
    type=float,
    default=1.0,
    help="scale factor for VID, i.e. mid_channels = sf * out_channels",
)
parser.add_argument(
    "--init_var", type=float, default=5.0, help="initial variance for VID"
)
parser.add_argument(
    "--att_f", type=float, default=1.0, help="attention factor of mid_channels for AFD"
)
...
def main():
    ...
    if args.kd_mode == "logits":
        ...
        for t_c in t_channels:
            criterionKD.append(
                AFD(t_c, args.att_f).cuda() if args.cuda else AFD(t_c, args.att_f)
            )
        criterionKD = [None] + criterionKD  # None is a placeholder
    ...
    if args.kd_mode in ["vid", "ofd", "afd"]:
        optimizer = torch.optim.SGD(
            chain(snet.parameters(), *[c.parameters() for c in criterionKD[1:]]),
            lr=args.lr,
        ...
        for i, (img, target) in enumerate(train_loader, start=1):
            ...
            if args.kd_mode in ["fsp"]:
                kd_loss = (
                    (
                        criterionKD(
                            stem_s[1], rb1_s[1], stem_t[1].detach(), rb1_t[1].detach()
                        )
                        + criterionKD(
                            rb1_s[1], rb2_s[1], rb1_t[1].detach(), rb2_t[1].detach()
                        )
                        + criterionKD(
                            rb2_s[1], rb3_s[1], rb2_t[1].detach(), rb3_t[1].detach()
                        )
                    )
                    / 3.0
                    * args.lambda_kd
                ...
                kd_loss = (
                    (
                        criterionKD(rb1_s[0], rb1_t[0].detach())
                        + criterionKD(rb2_s[0], rb2_t[0].detach())
                        + criterionKD(rb3_s[0], rb3_t[0].detach())
                    )
                    / 3.0
                    * args.lambda_kd
                )
...
def train(train_loader, nets, optimizer, criterions, epoch):
    ...
    for i, (img, target) in enumerate(train_loader, start=1):
        ...
        if args.kd_mode in ["logits", "st"]:
            ...
            kd_loss = (
                (
                    criterionKD(rb1_s[1], rb1_t[1].detach())
                    + criterionKD(rb2_s[1], rb2_t[1].detach())
                    + criterionKD(rb3_s[1], rb3_t[1].detach())
                )
                / 3.0
                * args.lambda_kd
            )
            ...
            kd_loss = (
                criterionKD(out_s, rb2_s[1], out_t, rb2_t[1], target) * args.lambda_kd
            )
            ...
            kd_loss = (
                (
                    criterionKD[1](rb1_s[1], rb1_t[1].detach())
                    + criterionKD[2](rb2_s[1], rb2_t[1].detach())
                    + criterionKD[3](rb3_s[1], rb3_t[1].detach())
                )
                / 3.0
                * args.lambda_kd
            )
            ...
            kd_loss = (
                (
                    criterionKD[1](rb1_s[0], rb1_t[0].detach())
                    + criterionKD[2](rb2_s[0], rb2_t[0].detach())
                    + criterionKD[3](rb3_s[0], rb3_t[0].detach())
                )
                / 3.0
                * args.lambda_kd
            )

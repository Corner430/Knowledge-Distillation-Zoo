...
class ContrastLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        log_neg = torch.div(
            neg_pair.clone().fill_(N / M), neg_pair.add(N / M + self.eps)
        ).log_()
...
class ContrastMemory(nn.Module):
    def __init__(self, feat_dim, n_data, nce_n, nce_t, nce_mom):
        ...
        self.register_buffer(
            "memory_t", torch.rand(n_data, feat_dim).mul_(2 * stdv).add_(-stdv)
        )
        self.register_buffer(
            "memory_s", torch.rand(n_data, feat_dim).mul_(2 * stdv).add_(-stdv)
        )

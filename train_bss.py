...
# hyperparameter
parser.add_argument(
    "--lambda_kd", type=float, default=2.0, help="trade-off parameter for kd loss"
)
parser.add_argument("--T", type=float, default=3.0, help="temperature for bss")
parser.add_argument(
    "--attack_size", type=int, default=32, help="num of samples for bss attack"
)
...
def train(train_loader, nets, optimizer, criterions, attacker, epoch):
    ...
    lambda_kd = (
        0
        if epoch <= 10
        else max(args.lambda_kd * (1 - 5 / 4 * (epoch - 1) / args.epochs), 0)
    )
        ...
        if lambda_kd > 0:
            ...
            if attack_flag.sum():
                ...
                if attack_idx.shape[0] > args.attack_size:
                    diff = (
                        F.softmax(out_t[attack_idx, :], 1)
                        - F.softmax(out_s[attack_idx, :], 1)
                    ) ** 2
                    score = (
                        diff.sum(dim=1)
                        - diff.gather(1, target[attack_idx].unsqueeze(1)).squeeze()
                    )
                    attack_idx = attack_idx[
                        score.sort(descending=True)[1][: args.attack_size]
                    ]

                # attack class selection
                attack_class = out_t.sort(dim=1, descending=True)[1][:, 1][attack_idx]
                class_score, class_idx = F.softmax(out_t, 1)[attack_idx, :].sort(
                ...
                rand_seed = (
                    torch.rand([rand_size]).cuda()
                    if args.cuda
                    else torch.rand([rand_size])
                )
                ...
                attacked_img = attacker.attack(
                    tnet, img[attack_idx, ...], target[attack_idx], attack_class
                )
